...
class SentinelAPI:
    ...
    def __init__(
        ...
        self.session = OAuth2Session(client=LegacyApplicationClient(client_id="cdse-public"))
        if user and password:
            id_url = "https://identity.dataspace.copernicus.eu/auth/realms/CDSE/protocol/openid-connect/token"
            self.session.auto_refresh_url = id_url
            self.session.auto_refresh_kwargs = {'client_id': "cdse-public"}
            self.session.token_updater = lambda x: None
            self.session.fetch_token(id_url, username=user, password=password, client_id="cdse-public")
        self.api_url = api_url if api_url.endswith("/") else api_url + "/"
        self.download_url = "https://download.dataspace.copernicus.eu/"
        self.page_size = 100
    ...
    def query(
        ...
        **keywords
        ...
        order_by: str, optional
            ...
            Example: "cloudCover, -beginposition".
        limit: int, optional
        ...
        **keywords
        ...
        Range values can be passed as two-element tuples, e.g. `cloudCover=(0, 30)`.
        ...
        self.logger.debug(
            "Running query: order_by=%s, limit=%s, page=%s, query=%s",
            order_by,
            limit,
            page,
            query,
        )
        response, count = self._load_query(query, order_by, limit, page, collection=collection)
    ...
    def format_query(area=None, date=None, raw=None, area_relation="Intersects", **keywords):
        ...
        if (
            ...
            or (date is not None and "startDate" in kw_lower)
            or (area is not None and "geometry" in kw_lower)
        ):
            raise ValueError(
        ...
        if date is not None:
            keywords["startDate"] = date[0]
            keywords["completionDate"] = date[1]
        ...
        if area is not None:
            query_parts.append('&geometry={}'.format(area))
    ...
    def _load_query(self, query, order_by=None, limit=None, page=1, collection="Sentinel2"):
        products, next_link = self._load_subquery(query, order_by, limit, page, collection=collection)
        ...
        while next_link is not None and (limit is None or limit > page * self.page_size):
            page += 1
            ret, next_link = self._load_subquery(query, order_by, limit, page, collection=collection)
            progress.update(len(ret))
            products += ret
        progress.close()
        return products, len(products)

    def _load_subquery(self, query, order_by=None, limit=None, page=1, collection="Sentinel2"):
        # store last query (for testing)
        self._last_query = query
        self.logger.debug("Sub-query: page=%s, limit=%s", page, limit)
        # load query results
        url = self._format_url(order_by, limit, page, query, collection=collection)
        ...
        try:
            ...
            if "detail" in json_feed:
                message = json_feed["detail"]["ErrorDetail"][0]["msg"]
                raise QuerySyntaxError(message, response)

            links = json_feed["properties"]["links"]
            links = list(filter(lambda x: x["rel"] == "next", links))
            next_link = links[0]["href"] if len(links) > 0 else None
        except (ValueError, KeyError):
            raise ServerError("API response not valid. JSON decoding failed.", response)

        products = json_feed.get("features", [])
        # this verification is necessary because if the query returns only
        # one product, self.products will be a dict not a list
        if isinstance(products, dict):
            products = [products]

        return products, next_link

    def _format_url(self, order_by=None, limit=None, page=1, query=None, collection="Sentinel2"):
        if limit is None:
            limit = self.page_size
        limit = min(limit, self.page_size)
        url = "search.json?maxRecords={}".format(limit)
        url += "&page={}".format(page)
        if order_by:
            url += "&sortParam={}".format(order_by)
        if query:
            url += query
        return urljoin(self.api_url, f"resto/api/collections/{collection}/{url}")
    ...
    def to_geojson(products):
        ...
        for i, (product_id, props) in enumerate(products.items()):
            props = props.copy()
            props["id"] = product_id
            poly = geomet.wkt.loads(props["geometry"])
            del props["geometry"]
            del props["gmlgeometry"]
    ...
    def to_geodataframe(products):
        ...
        geometry = [shapely.wkt.loads(fp) for fp in df["geometry"]]
        # remove useless columns
        df.drop(["geometry", "gmlgeometry"], axis=1, inplace=True)
    ...
    def get_product_odata(self, id, full=False):
        ...
        """
        url = self._get_odata_url(id, "?$select=*")
        if full:
        ...
        values["product_root_dir"] = values["title"]
        values["quicklook_url"] = self._get_odata_url(id, "/Products('Quicklook')/$value")
        values["url"] = self._get_odata_url(id, "/$value", True)
        return values
    ...
    def is_online(self, id):
        ...
        if not self._online_attribute_used:
            return True
        url = self._get_odata_url(id, "?$select=Online")
        with self.dl_limit_semaphore:
    ...
    def download(self, id, resolution=None, band=None, directory_path=".", checksum=True, nodefilter=None):
        """Download a product.
        ...
        return downloader.download(id, resolution, band, directory_path)
    ...
    def check_files(self, paths=None, ids=None, directory=None, delete=False):
        ...
        for id in ids:
            ...
            if name not in names_from_paths:
                paths.append(directory / odata["title"].replace(".SAFE", ".zip"))
    ...
    def _get_odata_url(self, uuid, suffix="", use_download_url=False):
        baseurl = self.download_url if use_download_url else self.api_url
        return baseurl + f"odata/v1/Products({uuid})" + suffix

    def _get_download_url(self, uuid):
        return self.download_url + f"odata/v1/Products({uuid})/$value"
    ...
    def _path_to_url(self, product_info, path, urltype=None):
        ...
        if urltype == "value":
            ...
            urltype = "?$expand=Attributes"
        elif urltype is None:
            urltype = ""
        # else: pass urltype as is
        return self._get_odata_url(id, f"/Nodes({root_dir})/{path}{urltype}", use_download_url=True)
    ...
    for prod in products:
        ...
        for key in prod:
            ...
            if key == "properties":
                ...
                for p in properties:
                    if p == "links":
                        for link in properties[p]:
                            name = "link"
                            if "rel" in link:
                                name = "link_" + link["rel"]
                            product_dict[name] = link["href"]
                    elif p == "services":
                        if "download" in properties[p]:
                            for d_prop in properties[p]["download"]:
                                product_dict["download_" + d_prop] = properties[p]["download"][d_prop]
                    elif isinstance(properties[p], dict):
                        for prop in properties[p]:
                            product_dict[f"{p}_{prop}"] = properties[p][prop]
                    else:
                        product_dict[p] = properties[p]
            elif key == "geometry":
                for g_prop in prod[key]:
                    product_dict[f"{key}_{g_prop}"] = prod[key][g_prop]
            else:
                product_dict[key] = prod[key]
    return output
...
def _parse_odata_response(product):
    output = {
        ...
        "PublicationDate": _parse_iso_date(product["PublicationDate"]),
