...
class Downloader:
    ...
    def download(self, id, directory=".", stop_event=None):
        ...
        filename = product_info["title"].replace(".SAFE", ".zip")
    ...
    def _download_band(self, id, path, product_info, resolution, band):
        filename = product_info["title"]
        level = filename.split("_")[1]
        url = self.api.download_url + f"/odata/v1/Products({id})/Nodes({filename})/Nodes(MTD_{level}.xml)/$value"
        self.logger.debug('Getting metadata info for tile \'%s\' remotelly', id)
        r = self.api.session.get(url)
        self.api._check_scihub_response(r, test_json=False)
        mcontents = r.content.decode("utf-8")

        if level == 'MSIL2A':
            rexp = "<IMAGE_FILE>GRANULE\/([0-9A-Z_]+)\/IMG_DATA\/R%sm\/([0-9A-Z_]+_%s_%sm)<\/IMAGE_FILE>" % (
                resolution, band, resolution)
        else:
            rexp = "<IMAGE_FILE>GRANULE\/([0-9A-Z_]+)\/IMG_DATA\/([0-9A-Z_]+_%sm)<\/IMAGE_FILE>" % band

        m = re.search(rexp, mcontents)
        if m is None:
            raise Exception("Could not find image metadata. uuid=%s, resolution=%s, band=%s" % (
                id, resolution, band))

        downloadFilename = path + "/products/%s/%s/%s.tiff" % (product_info["OriginDate"], id, m.group(2))
        if not os.path.exists(os.path.dirname(downloadFilename)):
            os.makedirs(os.path.dirname(downloadFilename))

        tmp_tile_filejp2 = "%s/tmp/%s.jp2" % (path, id)
        tmp_tile_filetiff = "%s/tmp/%s.tiff" % (path, id)
        if not os.path.exists(os.path.dirname(tmp_tile_filejp2)):
            os.makedirs(os.path.dirname(tmp_tile_filejp2))

        url = f"{self.api.download_url}/odata/v1/Products({id})/Nodes({filename})/Nodes('GRANULE')Nodes({m.group(1)})/Nodes('IMG_DATA')"

        if level == 'MSIL2A':
            url += f"/Nodes(R{resolution}m)"

        url += f"/Nodes({m.group(2)}.jp2)/$value"

        self.logger.info('Downloading tile uuid=\'%s\', resolution=\'%s\', band=\'%s\', date=\'%s\'', id,
                    resolution, band, product_info["OriginDate"])
        ...
        if band == 'TCI':
            self.logger.debug('Removing near black compression artifacts')
    ...
    def _download_with_node_filter(self, id, directory, stop_event):
        ...
        if not manifest_path.exists() and not self.api.is_online(id):
            if self.trigger_offline_retrieval(id):
                raise LTATriggered(id)
    ...
    def _init_statuses(self, product_ids):
        ...
        for pid in self._tqdm(
            ...
            try:
                info = self.api.get_product_odata(pid, full=True)
